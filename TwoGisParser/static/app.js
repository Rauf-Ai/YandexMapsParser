...
// ── Field definitions (16 fields — 2GIS schema) ──────────────────────────
const FIELD_DEFS = {
  name:        "Название",
  phone:       "Телефон",
  site:        "Сайт",
...
function applyTableFilters() {
  ...
  filteredRows = allCompanies.filter(c => {
    ...
    if (q) {
      const hay = [c.name, c.address, c.phone, c.category, c.services, c.features, c.hours]
        .join(" ").toLowerCase();
...
function renderTablePage() {
  ...
  tbody.innerHTML = slice.map((c, idx) => `
    <tr>
      ...
      <td class="td-hours">${c.hours ? esc(trunc(c.hours, 28)) : '<span class="td-muted">—</span>'}</td>
      <td>${renderTags(c.services, "srv")}</td>
      <td>${renderTags(c.features, "ftr")}</td>
      <td class="td-actions">
        ${c.map_url ? `<a href="${esc(c.map_url)}" target="_blank" rel="noopener" class="link-maps" title="Открыть в 2ГИС">🗺</a>` : ""}
...
function renderSocials(social) {
  ...
  const links = social.split(", ").filter(Boolean).map(s => {
    const icon = s.includes("vk.com") || s.includes("vkontakte") ? "VK" :
                 s.includes("t.me")   || s.includes("telegram")  ? "TG" :
                 s.includes("instagram")                          ? "IG" :
                 s.includes("ok.ru")                             ? "OK" :
                 s.includes("facebook")                          ? "FB" :
...
function startCollectSSE(jobId) {
  ...
  sse.addEventListener("done", e => {
    ...
    addCollectLog(`✓ Готово! Отзывов: ${d.reviews_count}, фото: ${d.photos_count}`, "ok");
    loadHistory();
  });
  sse.addEventListener("error", e => {
    sse.close(); currentCollectSSE = null;
